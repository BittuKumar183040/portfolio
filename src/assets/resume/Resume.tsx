...
const Resume = () => {
  return (
	<div className=" container m-auto max-w-[800px] p-2 flex flex-col gap-4 py-10">
    {/* Heading */}
      ...
      <div className=" flex flex-wrap justify-center mt-4">
        {
          Object.keys(details.contacts).map((contact:string, idx)=>
            <a className=" mx-2 underline text-blue-500" key={idx+contact} href={details.contacts[contact].url}>{details.contacts[contact].label}</a>
          )
        }
      </div>
    </section>
    {
      Object.entries(details.sections).map(([key, section]:[string, any]) =>
      <section key={key}>
        <h1 className=" text-xl font-bold tracking-wide">{key}</h1>
        <div className=" h-0.5 w-full bg-black my-2"></div>
        
          {section.map((contents:any, idx:number) => <div key={idx}>
            <div className=" flex justify-between items-center mt-2">
              ...
              {contents.details && 
                <div className=" relative group mx-2">
                  ...
                  <p className=" w-56 group-hover:scale-100 group-hover:opacity-100 flex flex-wrap gap-2
                    scale-0 opacity-0 transition-all absolute bottom-7 bg-opacity-50 backdrop-blur-sm 
                     bg-slate-100 shadow-md p-2 rounded-md text-xs md:text-sm">
                  
                  <span className=" absolute -bottom-4 left-1 opacity-60 border-8 border-t-gray-300 border-transparent"></span>
                  {contents.details.map((item:any, idx:any) => 
                    <span key={idx} className=" bg-slate-100 text-gray-600 px-2 rounded-sm shadow-inner" >{item}</span>
                  )}
                  </p>
                </div>
              }

              {contents.links && <div className=" flex gap-5">{
                Object.entries(contents.links).map(([key, link]:[key:string, link:any])=>
                <a className=" underline text-blue-500 text-xs md:text-sm" key={key} href={link}>{key}</a>    
                )}
              </div>
              }
            ...
            {contents.subtitle && 
              <div className=" flex justify-between items-center">
                <h1 className=" text-md font-medium">{contents.subtitle.title}</h1>
                <p className=" text-sm font-medium italic">{contents.subtitle.date}</p>
              </div>
            }
                  ...
                  <tbody key={idx}>
                    <tr>
                      {lists.map((item:string, idx:number) =>
                        <td key={idx} className=" text-justify py-1 relative" style={{width:"1px"}}>
                          <span className=" absolute -left-6 scale-50 filter grayscale">🔵</span>
